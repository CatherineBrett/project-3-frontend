...
export default function ShowTip({ user }: { user: null | IUser }) {
  ...
  useEffect(() => {
    async function fetchTip() {
      try {
        const response = await fetch(`${baseUrl}/tips/${tipId}`);
  ...
  async function deleteTip(e: SyntheticEvent) {
    try {
      ...
      await axios.delete(`${baseUrl}/tips/` + tipId, {
